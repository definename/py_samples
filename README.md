...
:point_up: `Присваивание` - в языке Python означает передаче ссылок на объекты, которые фактически реализованы в виде указателей. В зависимости от того переданный объект относится к категории `mutable` либо `immutable` изменения будут видны либо не видны из вне.
