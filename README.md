...
название которого состоит из первых букв названий областей видимости: Когда внутри функции выполняется обращение к неизвестному имени,  
интерпретатор пытается отыскать его в четырех областях видимости - в локальной (`local, L`), затем в локальной области любой объемлющей  инструкции def (`enclosing, E`) или в выражении lambda, затем в глобальной (`global, G`) и, наконец, во встроенной (`built-in, В`). Поиск завершается, как только будет найдено первое подходящее имя. Если требуемое имя не будет найдено, интерпретатор выведет сообщение об ошибке.

`global` означает, что переменная находятся в глобальной области видимости вмещающего модуля.

`nonlocal` означает, что переменная находятся в области видимости вмещающих функций.

## Concepts

:point_up: `Интерфейс` - набор методов и операций поддерживаемых объектом.
...
:point_up: `Замыкание` — это объект функции, который сохраняет значения в объемлющих областях видимости, даже если эти области прекратили свое существование.
